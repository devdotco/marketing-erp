...
import { META_STATE_COOKIE, metaAuthUrl, metaScopes } from "@/lib/integrations/meta";
import { integrationAdmin } from "@/lib/integrations/route-auth";

export const dynamic = "force-dynamic";

/** Top-level navigation from the Connect button → Facebook Login. */
export async function GET(req: NextRequest) {
  ...
  if (!metaScopes(provider)) return back("Unknown Meta integration");
  ...
  let url: string;
  try {
    url = metaAuthUrl(provider, nonce);
  } catch (err) {
  ...
  res.cookies.set(META_STATE_COOKIE, `${nonce}.${provider}.${who.workspaceId}`, {
    httpOnly: true,
